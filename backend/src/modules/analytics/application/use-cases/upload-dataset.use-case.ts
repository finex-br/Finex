...
export class UploadDatasetUseCase
  implements IUseCase<UploadDatasetRequestDTO, UploadDatasetResponseDTO>
{
  constructor(
  ...
  async execute(
    request: UploadDatasetRequestDTO,
  ): Promise<UploadDatasetResponseDTO> {
    console.log('[UploadDataset] Processing file:', request.fileName);
    ...
    console.log(`[UploadDataset] Parsed ${totalRows} rows, ${columns.length} columns`);

    // Create dataset entity
    const datasetResult = Dataset.create({
    ...
    const dataset = datasetResult.getValue();
    await this.datasetRepo.save(dataset);

    // Insert rows into dataset_rows (JSONB)
    ...
    console.log(`[UploadDataset] Dataset ${dataset.id.toString()} saved successfully`);
